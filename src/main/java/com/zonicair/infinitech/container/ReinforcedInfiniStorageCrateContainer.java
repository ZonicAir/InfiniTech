...
public class ReinforcedInfiniStorageCrateContainer extends Container {

    public final ReinforcedInfiniStorageCrateTileEntity tileEntity;
    private final IWorldPosCallable canInteractWithCallable;

    public ReinforcedInfiniStorageCrateContainer(final int windowID, final PlayerInventory playerInventory, final ReinforcedInfiniStorageCrateTileEntity tileEntity) {
        super(ContainerTypes.REINFORCED_INFINI_STORAGE_CRATE_CONTAINER.get(), windowID);
    ...
    private static ReinforcedInfiniStorageCrateTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data){
        Objects.requireNonNull(playerInventory, "playerInventory cannot be null");
        Objects.requireNonNull(data, "data cannot be null");
        final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if(tileAtPos instanceof ReinforcedInfiniStorageCrateTileEntity){
            return (ReinforcedInfiniStorageCrateTileEntity)tileAtPos;
        }
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(canInteractWithCallable, playerIn, BlockRegistryHandler.REINFORCED_INFINI_STORAGE_CRATE.get());
    }
